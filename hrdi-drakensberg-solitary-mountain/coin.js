	...
	let l = loader.load(
		...
		function (gltf) {
			...
			for (let i in coin.children[0].children) {
				...
				let assetEdgeGeometry = new THREE.EdgesGeometry(geometry, 30);
				let edgeMaterial = new THREE.LineBasicMaterial({ color: 0x000000 });
				let assetOutline = new THREE.LineSegments(assetEdgeGeometry, edgeMaterial);
				coinOutlineGroup.add(assetOutline)
			}
...
function setupBackground() {
	new RGBELoader().load(
		...
		'./assets/the_sky_is_on_fire_2k.hdr',
		function (texture) {
